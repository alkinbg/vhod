...
final readonly class CommunityInteractionService
{
    ...
    {
        return $this->entityManager->wrapInTransaction(function (EntityManagerInterface $entityManager) use ($author, $post, $body, $createdAt): CommunityComment {
    ...
    {
        return $this->entityManager->wrapInTransaction(function (EntityManagerInterface $entityManager) use ($user, $post, $type, $reactedAt): ?CommunityReaction {
            $entityManager->lock($post, LockMode::PESSIMISTIC_WRITE);
            self::assertPublished($post);
    ...
    {
        return $this->entityManager->wrapInTransaction(function (EntityManagerInterface $entityManager) use ($user, $poll, $option, $votedAt): CommunityPollVote {
