...
final class AnnouncementControllerTest extends WebTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $document = Document::record(
            ...
            'Документ към официалната обява.',
            'house-rules.pdf',
            str_repeat('a', 32).'.pdf',
            'application/pdf',
            100,
            $manager,
            new DateTimeImmutable('2026-09-09 06:00:00+00:00'),
        );
        $em->persist($document);

        $draft = OfficialAnnouncement::draft(
            'Чернова за ремонт',
            'Тази обява още не е публикувана.',
            $manager,
            new DateTimeImmutable('2026-09-09 07:00:00+00:00'),
        );
        $em->persist($draft);

        $older = OfficialAnnouncement::draft(
            'Проверка на асансьора',
            "Проверката е в петък.\nМоля, освободете достъпа.",
            $manager,
            new DateTimeImmutable('2026-09-09 07:10:00+00:00'),
            [$document],
        );
        ...
        $newer = OfficialAnnouncement::draft(
            'Спиране на водата',
            'Водата ще бъде спряна за кратка профилактика.',
            $manager,
            new DateTimeImmutable('2026-09-09 08:10:00+00:00'),
        );
        ...
        $historical = OfficialAnnouncement::draft(
            'Стара публикувана обява',
            'Достъпна е, но няма ретроактивно известие.',
            $manager,
            new DateTimeImmutable('2026-09-08 07:00:00+00:00'),
        );
        ...
        self::assertNotNull($older->getId());
        self::assertNotNull($newer->getId());
        self::assertNotNull($historical->getId());
        $this->documentId = $document->getId();
        $this->draftId = $draft->getId();
        $this->olderAnnouncementId = $older->getId();
        $this->newerAnnouncementId = $newer->getId();
        $this->historicalAnnouncementId = $historical->getId();
    }
    ...
    public function testAnonymousIsRedirectedAndDraftIsNotExposed(): void
    {
        ...
        $this->client->request('GET', '/announcement/'.$this->draftId);
    ...
    public function testResidentListsPublishedAnnouncementsNewestFirstAndSeesUnreadState(): void
    {
        ...
        self::assertStringNotContainsString('Чернова за ремонт', $content);
        self::assertStringContainsString('Спиране на водата', $content);
        self::assertStringContainsString('Проверка на асансьора', $content);
        self::assertStringContainsString('Стара публикувана обява', $content);
        self::assertLessThan(
            strpos($content, 'Проверка на асансьора'),
            strpos($content, 'Спиране на водата'),
        );
    ...
    {
        $this->client->loginUser($this->resident());
        $this->client->request('GET', '/announcement/'.$this->olderAnnouncementId);

        self::assertResponseIsSuccessful();
        self::assertSelectorTextContains('body', 'Официално');
        self::assertSelectorTextContains('body', 'Проверка на асансьора');
        self::assertSelectorTextContains('body', 'Правилник за вътрешния ред');
        self::assertSelectorExists('a[href="/document/'.$this->documentId.'/download"]');
        $content = (string) $this->client->getResponse()->getContent();
        self::assertStringContainsString('viber://forward?text=', $content);
        self::assertStringNotContainsString('Коментар', $content);
        self::assertStringNotContainsString('Харесва', $content);
    }

    public function testReadMutationIsCsrfProtectedAndIdempotent(): void
    {
        $this->client->loginUser($this->resident());
        $crawler = $this->client->request('GET', '/announcement/'.$this->olderAnnouncementId);
        self::assertResponseIsSuccessful();
        $form = $crawler->selectButton('announcement_read_submit')->form();
        $token = $form->get('_token')->getValue();
        self::assertIsString($token);

        $this->client->submit($form);
        self::assertResponseRedirects('/announcement/'.$this->olderAnnouncementId);

        $em = $this->entityManager();
        $em->clear();
        $receipt = $em->getRepository(AnnouncementReceipt::class)->findOneBy([
            'announcement' => $em->find(OfficialAnnouncement::class, $this->olderAnnouncementId),
            'user' => $em->find(User::class, $this->residentId),
        ]);
        self::assertInstanceOf(AnnouncementReceipt::class, $receipt);
        self::assertNotNull($receipt->getReadAt());
        $firstReadAt = $receipt->getReadAt();

        $this->client->request('POST', '/announcement/'.$this->olderAnnouncementId.'/read', ['_token' => $token]);
        self::assertResponseRedirects('/announcement/'.$this->olderAnnouncementId);
        $em->clear();
        $receipt = $em->getRepository(AnnouncementReceipt::class)->findOneBy([
            'announcement' => $em->find(OfficialAnnouncement::class, $this->olderAnnouncementId),
        ...
        self::assertInstanceOf(AnnouncementReceipt::class, $receipt);
        self::assertEquals($firstReadAt, $receipt->getReadAt());

        $this->client->request('POST', '/announcement/'.$this->newerAnnouncementId.'/read', ['_token' => 'invalid']);
    ...
    {
        $this->client->loginUser($this->resident());
        $this->client->request('GET', '/announcement/'.$this->historicalAnnouncementId);

        self::assertResponseIsSuccessful();
        self::assertSelectorTextContains('body', 'Стара публикувана обява');
        self::assertSelectorNotExists('button[name="announcement_read_submit"]');

        $em = $this->entityManager();
        $receipt = $em->getRepository(AnnouncementReceipt::class)->findOneBy([
            'announcement' => $em->find(OfficialAnnouncement::class, $this->historicalAnnouncementId),
            'user' => $em->find(User::class, $this->residentId),
        ]);
    ...
    {
        $this->client->loginUser($this->resident());
        $this->client->request('GET', '/announcement/'.$this->olderAnnouncementId.'/print');
        self::assertResponseIsSuccessful();
        self::assertResponseHeaderSame('content-type', 'text/html; charset=UTF-8');
        self::assertSelectorTextContains('body', 'Проверка на асансьора');
        self::assertSelectorTextContains('body', 'Правилник за вътрешния ред');

        $this->client->request('GET', '/announcement/'.$this->olderAnnouncementId.'/pdf');
        self::assertResponseIsSuccessful();
        self::assertResponseHeaderSame('content-type', 'application/pdf');
        self::assertResponseHeaderSame(
            'content-disposition',
            'attachment; filename=announcement-'.$this->olderAnnouncementId.'.pdf',
        );
    ...
    {
        $resident = $this->entityManager()->find(User::class, $this->residentId);
